...
Overwatch is McChrystal Group's AI-powered business development platform — and it's live now. You can explore the full working dashboard at [ace1523.github.io/bd-agent](https://ace1523.github.io/bd-agent/), where every prospect, dossier, outreach package, and market intelligence feed described in this guide is accessible and interactive.

The platform automates McChrystal Group's entire BD pipeline from first signal to final outreach. It starts with **Discovery**: Claude Code searches the web for organizations showing signs they need McChrystal Group — leadership transitions, mergers, organizational restructurings, rapid growth, activist investor pressure, earnings misses, and other trigger events. It scores each prospect against a detailed Ideal Customer Profile covering revenue range, employee count, industry tier, signal recency, and McChrystal-specific fit dimensions. The result is a ranked pipeline of companies with specific, current reasons to engage — not generic Fortune 500 names, but non-obvious targets like PE-backed roll-ups, fresh spinoffs, and niche industry leaders mid-transformation.

From there, the pipeline moves into **Research**. For every scored prospect, Claude Code builds a comprehensive 10-section dossier — the kind of briefing a Senior Partner can read in under 10 minutes and walk into a meeting fully prepared. Dossiers cover the organization snapshot, financial health, leadership team profiles (with priority outreach targets identified), culture and structure signals, recent trigger events, McChrystal fit assessment, conversation entry points, brand insights, and a deep strategic analysis that builds the full case for pursuit. That final section includes fit dimensions with competitor displacement logic, enterprise issues, expected engagement outcomes, a stakeholder map, and a phased land-and-expand strategy with dollar estimates.

Next comes **Outreach**. The platform generates three independent cold email versions (A/B/C) per prospect — trigger-based, insight-based, and warm-angle — each with a genuinely different opening strategy. These aren't sequential follow-ups; they're alternatives a Partner can choose from based on what feels right for the relationship. Each package also includes a LinkedIn message. Every email is 150–200 words, references something specific to that prospect, and targets a champion-level contact (VP/SVP/Head of Strategy) who feels the pain daily — not the CEO.

Running alongside all of this is **Market Intelligence** — a living feed covering 13 industry sectors (both niche categories like PE-backed roll-ups and defense consolidation, and general markets like financial services and technology). Each sector is refreshed with curated articles, key trends, McChrystal angles, and a "Companies to Watch" list that feeds back into the next discovery cycle.

The key architectural insight: **Claude Code is the entire engine.** There are no API integrations, no database, no CRM, no training pipelines. Claude Code performs live web research, reasons about prospect fit, builds structured Pydantic data models, generates Markdown reports, and pushes everything to a static dashboard hosted on GitHub Pages. The system is a Python library that Claude Code calls directly — every module is designed to be invoked conversationally, not through a CLI or scheduled jobs. A single instruction can trigger the full pipeline end-to-end: discover, score, research, draft outreach, update the dashboard, and deploy.

But there's a deeper pattern at work here: **the filesystem *is* the agent.** Overwatch has no database, no message broker, no container orchestration, no orchestrator process. The entire runtime is three directories. `bd/` is the logic layer — Python modules that define scoring, report generation, data models, and persistence. `data/` is the persistence layer — Markdown reports for human readability and JSON for machine state. `docs/` is the presentation layer — a static single-page application that reads `dashboard.json` and renders everything in the browser. The Unix filesystem provides what most agent frameworks build from scratch: `fcntl.flock` for file-level concurrency control, JSON files for state management, Markdown for human-readable output, and git for versioning, audit trail, and deployment (push to main → GitHub Pages updates automatically).

This inverts the typical AI agent architecture. Most frameworks wrap LLMs in layers of scaffolding — vector databases for retrieval, tool-calling APIs, prompt chaining engines, orchestration layers, and custom infrastructure to manage state between steps. Overwatch doesn't need any of that because Claude Code already has the capabilities baked in: web search, file I/O, code execution, and multi-step reasoning. So instead of building infrastructure *around* the model, Overwatch builds a well-organized project directory that Claude Code navigates *naturally*. The instructions live in `CLAUDE.md`, the logic lives in Python modules Claude Code can import and call, and the output lives in flat files that are both human-readable and machine-parseable. There is no training, no fine-tuning, no prompt chains, no API keys. The system is the filesystem — and the filesystem is the system.

This pattern is reproducible. Anyone can build a domain-specific AI agent the same way: structure a project directory with clear separation between logic, data, and presentation; write Python modules that Claude Code can call to do the domain-specific work; put the workflow instructions in `CLAUDE.md`; and let Claude Code orchestrate. The result is an agent that's auditable (every output is a file in git), debuggable (read the Markdown), and deployable (push to main) — with zero infrastructure to maintain.

This guide is the complete technical reference for how it all works. For a hands-on look at what the platform produces, start with the [live dashboard](https://ace1523.github.io/bd-agent/) — browse the pipeline, expand a dossier, read the outreach drafts, and explore the market intelligence feed. Then come back here for the architecture, data models, scoring logic, and implementation details behind it all.
