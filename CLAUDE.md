...
- **Connections** — LinkedIn connection intelligence from ATL_BD enrichment project. Embedded via iframe (`connections.html?embed=1`) with its own light theme. Region sub-tabs (All Regions, DC, Atlanta, NYC, Chicago — NYC/CHI are placeholders). Three data views per region:
  - **Executive Connections** — leaders with direct MG Point of Contact (column D from enriched Excel). Sortable by company, leader, role, MG contact
  - **All Company Connections** — every LinkedIn connection at target companies, grouped by MG partner. Sortable by company, connection, position, partner
  - **Military Affiliations** — military-affiliated executives at target companies with rank, branch, and service details
  - Partner filter pills filter across all three views. Data sourced from `connections.json` generated by `import_linkedin.py`
